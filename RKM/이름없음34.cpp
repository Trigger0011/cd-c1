...
bool monsterAttack(Player &player, Monster &monster) {
    ...
    cout << monster.name << "의 공격! " << monster.attack << " 데미지를 받았다!\n";
    return player.hp <= 0;
}
...
void shop(Player &player) {
    ...
    while (true) {
        ...
        else if (choice == 3 && player.gold >= 5) { player.critChance += 1; player.gold -= 5; }
        else if (choice == 4 && player.gold >= 15) { player.critDamage += 10; player.gold -= 15; }
...
int main() {
    ...
    Monster monsters[10] = {
        {"겁나게 빠른 슉슈슉 스피드한 귀여운  고양이 드래곤 마왕 슬라임", 5, 15, 0.9},
        {"고블린", 50, 8, 0.0},
        {"늑대", 70, 10, 0.0},
        {"트롤", 100, 12, 0.0},
        {"오우거", 130, 15, 0.0},
        ...
        {"기사", 200, 20, 0.0},
        {"드래곤", 300, 25, 0.0},
        {"*[저주의 왕]* 스쿠나", 400, 80, 0.5, 4}, // 빨강
        {"*[무하한]* 고죠 사토루", 100, 100, 0.8, 1} // 파랑
    };
